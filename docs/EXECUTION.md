...
- `-PrailcraftJar=/path/to/railcraft-reborn-1.21.1-1.2.10.jar`
...
bash gallery/package.sh /tmp/railcraft-gallery.zip
```
...
Record `visual_acceptance: true` under `owner_accepted_staging`, change the
provenance status from `unpublished-migration-candidate` to
`owner-accepted-release-candidate`, and retain the exact production JAR,
sources JAR, POM, and Gradle module identities under `candidate_artifacts`.

Run with all exact candidate properties:
